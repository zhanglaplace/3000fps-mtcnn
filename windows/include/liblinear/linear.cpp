...
// A coordinate descent algorithm for 
// the dual of L2-regularized LOGGistic regression problems
//
//  min_\alpha  0.5(\alpha^T Q \alpha) + \sum \alpha_i LOGG (\alpha_i) + (upper_bound_i - \alpha_i) LOGG (upper_bound_i - \alpha_i),
//    s.t.      0 <= \alpha_i <= upper_bound_i,
// 
//  where Qij = yi yj xi^T xj and 
//  upper_bound_i = Cp if y_i = 1
...
void solve_l2r_lr_dual(const problem *prob, double *w, double eps, double Cp, double Cn)
{
	...
	while (iter < max_iter)
	{
		...
		for (s=0; s<l; s++)
		{
			...
			//  g_t(z) = z*LOGG(z) + (C-z)*LOGG(C-z) + 0.5a(z-alpha_old)^2 + sign*b(z-alpha_old)
...
// A coordinate descent algorithm for 
// L1-regularized LOGGistic regression problems
//
//  min_w \sum |wj| + C \sum LOGG(1+exp(-yi w^T xi)),
//
// Given: 
// x, y, Cp, Cn
// eps is the stopping tolerance
