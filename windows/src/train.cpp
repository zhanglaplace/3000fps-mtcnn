...
#define SWAP(shape, i, j) do { \
    ...
    if (config.landmark_n == 29) {
        ...
        LOGG("Wrang Landmark_n, it must be 29 or 68");
    }
...
int train(int start_from) {
    ...
    TIMER_BEGIN
        ...
        LOGG("Train Model Down, cost %.4lf s", TIMER_NOW);
    TIMER_END
