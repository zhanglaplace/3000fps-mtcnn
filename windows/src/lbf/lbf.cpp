...
void LbfCascador::Train(vector<Mat> &imgs, vector<Mat> &gt_shapes, vector<Mat> &current_shapes, \
    ...
    for (int k = start_from; k < stages_n; k++) {
        ...
        // global regression
        LOGG("start train global regression of %dth stage", k);
        TIMER_BEGIN
            GlobalRegressionTrain(lbfs, delta_shapes, k);
            LOGG("end of train global regression of %dth stage, costs %.4lf s", k, TIMER_NOW);
        TIMER_END
            // update current_shapes
            double scale;
        Mat rotate;
        ...
        // calc mean error
        double e = calcMeanError(gt_shapes, current_shapes);
        LOGG("Train %dth stage Done with Error = %lf", k, e);
...
void LbfCascador::DumpTrainModel(int stage) {
    ...
    LOGG("Dump model of stage %d", stage);
    char buff[50];
...
void LbfCascador::ResumeTrainModel(int start_from) {
    ...
    LOGG("Resuming training from stage %d", start_from);
...
void LbfCascador::Test(vector<Mat> &imgs, vector<Mat> &gt_shapes, vector<BBox> &bboxes) {
    ...
    TIMER_BEGIN
        ...
        for (int k = 0; k < stages_n; k++) {
            ...
            LOGG("stage %dth error = %.6lf", k, e);
        }
        LOGG("Test %d images costs %.4lf s with %.2lf fps", N, TIMER_NOW, N / TIMER_NOW);
    TIMER_END
}
